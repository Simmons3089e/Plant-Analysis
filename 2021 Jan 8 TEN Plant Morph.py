...
class options:
    def __init__(self):
        ...
        self.debug = "plot"
        self.writeimg= False 
        self.result = "./morphology_tutorial_results.json"
        self.outdir = "."
...
s_thresh = pcv.threshold.binary(gray_img=s, threshold=166, max_value=255, object_type='light')
...
b_cnt = pcv.threshold.binary(gray_img=b, threshold=133, max_value=255, object_type='light')
...
roi1, roi_hierarchy= pcv.roi.rectangle(img=img, x=50, y=300, h=50, w=50)
...
img1, seg_img, edge_objects = pcv.morphology.prune(skel_img=skeleton, size=9, mask=cropped_mask)
